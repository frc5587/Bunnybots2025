...
public class LEDController extends SubsystemBase {
  ...
  private double snakeSpeedLedsPerSecond = 40.0;
  ...
  public static enum LEDColor { // Define some preset colors
    TR_RED(190, 32, 38),
    TR_BLUE(42, 57, 124),
    GREEN(0, 255, 0),
    YELLOW(255, 255, 0),
  ...
  @Override
  // Calling of methods (patterns) can be omitted from here if they're called as
  // inputs
  ...
  // - LEDController.applyProgress(LEDColor.TR_RED, 0.5) will light up half the
  // strip red
  ...
  /**
   * Progressive light up method - LEDController.applyProgress(255, 0, 0, 0.5)
   * will light up half the strip red
   * 
   * @param r          Red component (0-255), g Green component (0-255), b Blue
   *                   component (0-255)
   * @param m_progress Progress value from 0.0 to 1.0 indicating how much of the
   *                   strip to light up
   */
  private void applyProgress(int r, int g, int b, double m_progress) {
    if (!colorProgressActive) {
      m_timer.reset();
      m_timer.start();
      colorProgressActive = true;
    } // Flag for one-time progress start
  ...
  /**
   * Solid color method - LEDController.applyColorSolid(255, 0, 0) will set the
   * whole strip red
   * 
   * @param r          Red component (0-255), g Green component (0-255), b Blue
   *                   component (0-255)
  ...
  // - LEDController.applyColorBlink(LEDColor.TR_RED, LEDColor.OFF, 10.0) will
  ...
  /**
   * Blinking color method - LEDController.applyColorBlink(255, 0, 0, 0, 0, 0,
   * 10.0) will blink red for 10 seconds
   * 
   * @param r               Red component of first color (0-255), g Green
   *                        component (0-255), b Blue component (0-255)
   * @param r2              Red component of second color (0-255), g2 Green
   *                        component (0-255), b2 Blue component (0-255)
  ...
  // - LEDController.startSnakeAnimation(LEDColor.TR_RED, LEDColor.OFF, true) will
  // start a red snake on a black background
  ...
  /**
   * Starts a snake-like animation that moves a short lit segment across the
   * strip.
   * - LEDController.startSnakeAnimation(255, 0, 0, 0, 0, 0, 40.0, true) will
   * start a red snake on a black background
   *
   * @param r             Red component of the snake color (0-255), g Green
   *                      component (0-255), b Blue component (0-255)
   * @param r2            Red component of the background color (0-255), g2 Green
   *                      component (0-255), b2 Blue component (0-255)
